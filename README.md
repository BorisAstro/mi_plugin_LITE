...
## Scripts nuevos registrados

- fix_geojson.py — normaliza left_line_id / right_line_id / id
- migrate_fields.py — speed→speed_limit, direction→turn_direction sin sobrescribir
- extractimagetopodot.py — EO de imágenes desde *.lst a GPKG

---

# v1.0.0 — Plugin autocontenido

Todos los scripts viven ahora DENTRO del plugin, en `scripts/`. La interfaz
está cableada directamente a ellos: lo que eliges en la UI es lo que usa el
script (adiós al problema de removepath).

**17 scripts ya funcionales** (reescritos y cableados a la UI): removepath,
rutasgeojson, revisarids_duplicados, rellenoid, distancias, reportetxt_ids,
fix_geojson, migrate_fields, convertir_crs, aplicar_estilos, streetview2,
drape_to_3d, layer3d_a_2d, shp_to_geojson_with_reference, offset2,
laneletcount, box_to_point_dem.

**8 slots pendientes de tu código original** (lógica de dominio que solo está
en tus archivos): calcular_width_height_geojson, focos_semaforo, topologia,
lanelet_val, lanelet_merge (convierte antes el .ipynb con nbconvert),
laneletqc, cortefinal_consolidado2, extractimagetopodot. Para completarlos:
selecciona el script en el plugin y pulsa **Importar script…** — copia tu .py
original al slot. Luego envuelve su configuración con
`if "VAR" not in globals():` para que tome los valores de la interfaz.

Convención de nombres aplicada en fix_geojson y migrate_fields a nivel de
ARCHIVO: el corregido/migrado conserva el nombre base y el original pasa a
`<base>_fix.geojson` / `<base>_migrate.geojson` (sin backups *_fixed). Las
capas cargadas que apuntan al archivo se recargan solas.

Ediciones futuras: los scripts instalados están en
`...\QGIS4\profiles\default\python\plugins\mi_plugin\scripts\` — edítalos ahí
y los cambios aplican en la siguiente ejecución (sin recargar el plugin).
OJO: reinstalar el plugin desde ZIP reemplaza la carpeta; guarda copia de los
scripts que importes o edites.
